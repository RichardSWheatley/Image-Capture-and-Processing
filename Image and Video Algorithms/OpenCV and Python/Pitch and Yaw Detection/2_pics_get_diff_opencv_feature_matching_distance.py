...
def runImageProcessing(img_1, img_2, detector, type, startTime, ref_heading, delta_pixels_h, delta_pixels_w):
    ...
    if(len(good) > 0):
        ...
        degrees = True
        if degrees == True:
            cv2.putText(img_1, my_pitch_string + "{:10.4f}".format(pitch*0.075), (30,30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (0,0,250), 1, cv2.LINE_AA);
            cv2.putText(img_1, my_heading_string + "{:10.4f}".format(heading*0.075), (30,60), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (0,0,250), 1, cv2.LINE_AA);
